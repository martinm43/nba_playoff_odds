...
mcrows = []
with open(phone+'outfile_mcsims.csv','rb') as csvfile:
...
#get known wins
winrows = []
with open(phone+'outfile_wins.csv','rb') as csvfile:
	balldata = csv.reader(csvfile,delimiter=',')
	for row in balldata:
		winrows.append(row)
	csvfile.close	
...
ite=input('Enter the number of desired simulations:')
...
csvfile_out = open(phone+'MC_sim_results_Excel.csv','wb')
...
for i in range(1,31):
	print('Team '+id_to_name(i,teamdict)+' has a playoff probability of '+'{0:.2%}'.format(float(playoff_results.count(i))/float(ite)))
