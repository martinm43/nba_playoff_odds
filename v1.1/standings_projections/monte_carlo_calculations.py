...
app_dir='/home/martin/naismith/v1.1/analytics/'
import sys
sys.path.append(app_dir)

from morey import SRS_regress,burke_regress

srs_dir='/home/martin/naismith/v1.1/analytics/'
import sys
sys.path.append(srs_dir)
...
projdata=[]
with open(phone+'outfile_future_games.csv','rb') as futurefile:
...
model_selection=raw_input('Please enter the type of model to be applied :')
...
with open(phone+model_csv,'rb') as srsfile:
...
csvfile_out = open(phone+'outfile_mcsims.csv','wb')
...
#pprint(fancy_out)
csvfile_out = open(phone+'coming_games_Excel.csv','wb')
csvwriter = csv.writer(csvfile_out)
csvwriter.writerow(['Date','Home Team','Away Team','dSRS','Away Team Win Probability'])
for row in fancy_out:
	#Only need to print the visiting and home team scores and names.
	csvwriter.writerow(row)
csvfile_out.close()

#write out the "games won" file - a process
regrows = []
with open(phone+'outfile_past_games.csv','rb') as csvfile:
...
for row in regrows:
	...
	if row[1]=='' or row[3]=='':
		...
		#Even if you think you don't have to convert to integers, you actually do!!
		if int(row[3])>int(row[1]):
			list_of_winners.append(row[2]) 
		elif int(row[1])>int(row[3]):
			list_of_winners.append(row[0])
	

#Count the occurences of each team in the list of winners and 
#then append it to a matrix we will print for MATLAB (and other programs) use
#list_of_winners=[] #for season prediction testing
winlist=[]
for i in range(1,31):
	winlist.append(list_of_winners.count(str(i)))
#Convert to "list of lists"
winlist = zip(*[iter(winlist)]*1)

csvfile_out = open(phone+'outfile_wins.csv','wb')
csvwriter = csv.writer(csvfile_out)
for row in winlist:
	#Only need to print the visiting and home team scores and names.
	csvwriter.writerow(row)
...
##I think this is good.. - MAM Jan 17
